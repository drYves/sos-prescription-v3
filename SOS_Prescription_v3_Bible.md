...
- **Worker (Node.js)** : exécution asynchrone des tâches lourdes (PDF), archivage batch des logs, endpoint /pulse.
...
**Stockage nonce (recommandé)** :
- Redis (TTL) si disponible et si scaling > 1 instance
- sinon : table dédiée (ou “outbox” limitée) MySQL
...
**Bucket PDF** : privé, jamais public.

**Object keys (sans PII)** :
- `unit/<site_id>/rx-pdf/<yyyy>/<mm>/<job_id>.pdf`
- `unit/<site_id>/rx-pdf/<yyyy>/<mm>/<job_id>.meta.json`
...
Keys recommandées :
- `logs/<site_id>/worker/<yyyy>/<mm>/<dd>/critical_<hhmm>_<worker_id>_seqNNN.ndjson.gz`
- `logs/<site_id>/web/<yyyy>/<mm>/<dd>/audit_<hhmm>_<instance>_seqNNN.ndjson.gz`
...
**Décision v3 (par défaut)** : presigned URL.
...
- TTL court (60s) auto-invalidation
...
Exemple :
```json
{"ts":"2026-02-08T12:34:56.789Z","ts_ms":1760003696789,"severity":"warning","component":"web","service":"sosprescription","site_id":"wp_site_01","env":"prod","event":"rx.pdf.download.requested","req_id":"R8K4P2","context":{"rx_id":12345,"job_status":"PENDING"}}
```

### 5.3 Logging “Dual-Layer”
**Layer 1 — Live**
- Web + Worker loguent en NDJSON vers stdout/stderr.
- Scalingo capture et exporte via log drain (destination HDS recommandée).
...
**Concurrence PDF**
- 1 job lourd à la fois par Worker (Puppeteer/PDF) par défaut.
...
### 7.2 Lazy Migration (Option C — recommandée)
...
### 7.3 Variante “filet de sécurité” (optionnelle)
