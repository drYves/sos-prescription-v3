...
export interface PolishMessageConstraints {
  audience?: "patient" | "doctor" | "internal";
  tone?: "professional" | "warm" | "direct" | "reassuring";
  language?: string;
  maxCharacters?: number;
  preserveDecision?: boolean;
  forceClarificationIfAmbiguous?: boolean;
}

interface NormalizedPolishConstraints {
  audience: "patient" | "doctor" | "internal";
  tone: "professional" | "warm" | "direct" | "reassuring";
  language: string;
  maxCharacters?: number;
  preserve_decision: boolean;
  force_clarification_if_ambiguous: boolean;
}
...
export interface SmartReplyThreadEntry {
...
export interface GenerateSmartRepliesInput {
  ...
  threadPreview?: SmartReplyThreadEntry[];
  maxCharactersPerReply?: number;
}

export type SmartReplyType = "clarification" | "confirmation" | "refus_poli";
...
export interface GenerateSmartRepliesResult {
...
export class CopilotService {
  ...
  constructor(cfg: CopilotServiceConfig) {
  ...
  async polishMessage(draft: string, constraints: PolishMessageConstraints = {}): Promise<PolishMessageResult> {
    ...
    const response = await this.requestJson<{ rewritten_body?: unknown; changes_summary?: unknown; risk_flags?: unknown }>({
    ...
    const result: PolishMessageResult = {
  ...
  async generateSmartReplies(input: GenerateSmartRepliesInput): Promise<GenerateSmartRepliesResult> {
    ...
    const response = await this.requestJson<{ replies?: unknown; risk_flags?: unknown }>({
  ...
  private async requestJson<T extends Record<string, unknown>>(input: {
    systemPrompt: string;
    userPayload: Record<string, unknown>;
    operation: string;
    temperature: number;
    maxTokens: number;
  }): Promise<{ parsed: T; provider?: string; model?: string }> {
    if (!this.apiKey) {
    ...
    try {
      ...
      let parsedBody: OpenRouterChatResponse | null = null;
      try {
        parsedBody = rawBody ? (JSON.parse(rawBody) as OpenRouterChatResponse) : null;
      } catch {
        parsedBody = null;
      }
      ...
      const parsed = parseJsonObject<T>(assistantContent);
      this.logger?.info(
...
function normalizePolishConstraints(input: PolishMessageConstraints): NormalizedPolishConstraints {
  const out: NormalizedPolishConstraints = {
...
function normalizeSmartRepliesInput(input: GenerateSmartRepliesInput): Required<GenerateSmartRepliesInput> {
  return {
    ...
    threadPreview: (Array.isArray(input.threadPreview) ? input.threadPreview : [])
      ...
      .filter((row) => row.body !== "")
      .slice(-6),
    maxCharactersPerReply:
      typeof input.maxCharactersPerReply === "number" && Number.isFinite(input.maxCharactersPerReply) && input.maxCharactersPerReply > 30
        ? Math.trunc(input.maxCharactersPerReply)
        : DEFAULT_SMART_REPLY_MAX_CHARS,
  };
}

function normalizeSmartReplyOptions(value: unknown, maxCharacters: number): SmartReplyOption[] {
...
function dedupeReplyTypes(replies: SmartReplyOption[]): SmartReplyOption[] {
  const out: SmartReplyOption[] = [];
  const seen = new Set<SmartReplyType>();
...
function hasAllReplyTypes(replies: SmartReplyOption[]): boolean {
...
function buildFallbackSmartReplies(
  input: Required<GenerateSmartRepliesInput>,
  riskFlags: string[] = [],
): GenerateSmartRepliesResult {
    ...
    replies: [
      {
        type: "clarification",
        title: "Clarification",
        body: applyMaxCharacters(
          `Bonjour, pouvez-vous préciser votre demande concernant ${medicationHint} et, si besoin, joindre l'ordonnance ou indiquer la posologie habituelle ?`,
          input.maxCharactersPerReply,
        ),
      },
      {
        type: "confirmation",
        title: "Confirmation",
        body: applyMaxCharacters(
          `Bonjour, votre message a bien été reçu concernant ${medicationHint}. Je vérifie votre dossier et je reviens vers vous rapidement.`,
          input.maxCharactersPerReply,
        ),
      },
      {
        ...
        title: "Refus poli",
        body: applyMaxCharacters(
          `Bonjour, je ne peux pas confirmer cette demande en l'état. Merci de préciser le contexte ou de transmettre les éléments nécessaires afin que je puisse vous répondre correctement.`,
          input.maxCharactersPerReply,
        ),
      },
    ],
...
function normalizeSmartReplyType(value: unknown): SmartReplyType | null {
...
function defaultTitleForType(type: SmartReplyType | null): string {
  switch (type) {
    case "clarification":
      return "Clarification";
    case "confirmation":
...
function normalizeStringArray(value: unknown, maxItems = 12, fallbackItem?: string): string[] {
...
function uniqueCleanStrings(value: string[], maxItems: number): string[] {
...
function applyMaxCharacters(value: string, maxCharacters?: number): string {
...
function normalizeDraftInput(value: unknown): string {
...
function parseJsonObject<T extends Record<string, unknown>>(rawContent: string): T {
