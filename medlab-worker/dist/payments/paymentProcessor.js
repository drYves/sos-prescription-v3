...
exports.processPaymentActionJob = processPaymentActionJob;
const stripeClient_1 = require("./stripeClient");
const wordpressPaymentBridge_1 = require("./wordpressPaymentBridge");
async function processPaymentActionJob(job, deps) {
    ...
    try {
        ...
        deps.logger.info("payment.job.done", {
            job_id: job.id,
            kind: job.kind,
        ...
        if (classification.retryable) {
            ...
            deps.logger.warning("payment.job.requeued", {
                job_id: job.id,
                kind: job.kind,
                payment_intent_id: job.paymentIntentId,
        ...
        deps.logger.error("payment.job.failed", {
            job_id: job.id,
            kind: job.kind,
            payment_intent_id: job.paymentIntentId,
            code: classification.code,
...
async function resolveIntentState(job, currentIntent, stripe) {
    if (job.kind === "payment.capture") {
        ...
        throw new stripeClient_1.StripeGatewayError("ML_STRIPE_NOT_CAPTURABLE", `PaymentIntent is not capturable yet (${currentIntent.status || "unknown"}).`, true, null, { status: currentIntent.status });
    }
    if (currentIntent.status === "canceled") {
        return currentIntent;
    }
...
function classifyError(err, job) {
    ...
    if (err instanceof wordpressPaymentBridge_1.WordPressPaymentBridgeError) {
        return {
            retryable: err.transient,
            code: err.code,
            message: safeMessage(err.message, job.kind),
...
function safeMessage(message, kind) {
