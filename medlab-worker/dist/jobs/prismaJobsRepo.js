...
class PrismaJobsRepo {
    ...
    constructor(cfg) {
        ...
        this.wpCallbackPathTemplate = normalizePathTemplate(cfg.wpCallbackPathTemplate ?? process.env.WP_SHADOW_CALLBACK_PATH_TEMPLATE ?? DEFAULT_WP_CALLBACK_PATH_TEMPLATE, DEFAULT_WP_CALLBACK_PATH_TEMPLATE);
        ...
        for (let attempt = 1; attempt <= 5; attempt += 1) {
            try {
                ...
                if (created.mode === "created") {
                    this.logger?.info("ingest.accepted", {
                    ...
                    this.logger?.info("ingest.replayed", {
                        job_id: created.job_id,
                        prescription_uid: created.uid,
                        processing_status: created.processing_status,
                        source_req_id: created.source_req_id,
    ...
    async claimNextPendingJob(opts) {
        ...
        if (nowMs - this.lastSweepAtMs >= 30_000) {
            ...
            try {
                ...
                this.logger?.warning("db.job.sweep_failed", { message: err instanceof Error ? err.message : "sweep_failed" }, undefined);
            }
        }
      ...
      `;
        ...
        this.logger?.info("db.job.claimed", {
            job_id: job.job_id,
    ...
    async markDone(opts) {
        ...
        this.logger?.info("db.job.done", {
            job_id: opts.jobId,
            worker_ref: workerRef,
            s3_key_ref: opts.s3KeyRef,
            artifact_size_bytes: opts.artifactSizeBytes,
        }, opts.reqId);
        await this.notifyWordPressShadow(this.buildCallbackEnvelope({
            reqId: opts.reqId,
            job: {
                job_id: opts.jobId,
    ...
    async markFailed(opts) {
        ...
        this.logger?.error("db.job.failed", {
            job_id: opts.jobId,
            worker_ref: workerRef,
    ...
    async requeueWithBackoff(opts) {
        ...
        this.logger?.warning("db.job.requeued", {
            job_id: opts.jobId,
            worker_ref: workerRef,
        ...
        }), opts.jobId, opts.reqId, "requeued");
    }
    async getQueueMetrics(siteId) {
    ...
    async sweepZombies(siteId, limit = 50) {
        if (siteId !== this.siteId) {
    ...
    mapClaimedRowToJob(row) {
        ...
        const mls1Token = (0, mls1_1.buildMls1Token)(Buffer.from(payloadJson, "utf8"), this.hmacSecretActive);
        return {
            id: row.id,
            job_id: row.id,
            site_id: this.siteId,
    ...
    buildCallbackEnvelope(input) {
        return {
            ...
            site_id: this.siteId,
            ts_ms: Date.now(),
            nonce: (0, mls1_1.base64UrlEncode)((0, node_crypto_1.randomBytes)(16)),
            req_id: normalizeNonEmptyString(input.reqId, generateReqId()),
            job: input.job,
        };
    }
    async notifyWordPressShadow(body, jobId, reqId, scope) {
        if (this.wpBaseUrl === "") {
            this.logger?.warning("wp.shadow_callback.skipped", { job_id: jobId, reason: "wp_base_url_missing", scope }, reqId);
            return;
        }
        const path = renderPathTemplate(this.wpCallbackPathTemplate, jobId);
        const url = `${this.wpBaseUrl}${path}`;
        const rawJson = JSON.stringify(body);
        const token = (0, mls1_1.buildMls1Token)(Buffer.from(rawJson, "utf8"), this.hmacSecretActive);
        for (let attempt = 1; attempt <= DEFAULT_CALLBACK_RETRIES; attempt += 1) {
            ...
            try {
                ...
                this.logger?.info("wp.shadow_callback.accepted", {
                    job_id: jobId,
                    scope,
                ...
                if (attempt >= DEFAULT_CALLBACK_RETRIES) {
                    this.logger?.error("wp.shadow_callback.failed", {
                        job_id: jobId,
                        scope,
                        path,
                        attempts: attempt,
                ...
                this.logger?.warning("wp.shadow_callback.retry", {
                    job_id: jobId,
                    scope,
                    attempt,
                    error: message,
...
exports.PrismaJobsRepo = PrismaJobsRepo;
...
function mapIngestResult(row, mode, reqId) {
    return {
        mode,
        job_id: row.id,
        prescription_id: row.id,
...
function mapApproveResult(row, reqId) {
    return {
        mode: "approved",
        job_id: row.id,
        prescription_id: row.id,
...
function mapRejectResult(row, reqId) {
    return {
        mode: "rejected",
        job_id: row.id,
        prescription_id: row.id,
...
function buildDoctorCreate(input) {
    return {
...
function buildDoctorUpdate(input) {
    return {
        firstName: normalizeNullableString(input.firstName),
        lastName: normalizeNullableString(input.lastName),
        email: normalizeNullableString(input.email),
...
function buildPatientCreate(input) {
    return {
...
function canonicalizePrescriptionItems(items) {
    return items.map((item, index) => canonicalizePrescriptionItem(item, index));
}
function canonicalizePrescriptionItem(item, index) {
...
function normalizeSchedulePayload(value) {
...
function scheduleToCanonicalText(schedule) {
...
function scheduleToDurationLabel(schedule) {
    const durationVal = toPositiveInt(schedule.durationVal ?? schedule.durationValue ?? schedule.duration);
    const durationUnit = normalizeFrequencyUnit(schedule.durationUnit ?? schedule.unit, true);
...
function normalizeFrequencyUnit(value, allowMonth = true) {
...
function generatePublicUid() {
    ...
    const bytes = (0, node_crypto_1.randomBytes)(UID_LENGTH);
    let out = "";
...
function generateVerifyToken() {
    return (0, mls1_1.base64UrlEncode)((0, node_crypto_1.randomBytes)(VERIFY_TOKEN_BYTES));
}
function generateReqId() {
    return `req_${(0, mls1_1.base64UrlEncode)((0, node_crypto_1.randomBytes)(8))}`;
}
...
function normalizeRequiredInt(value, field) {
...
function assertIngestRequest(input, siteId) {
...
function renderPathTemplate(template, jobId) {
    return template
