...
exports.MessagesRepo = exports.MessagesRepoError = void 0;
...
class MessagesRepoError extends Error {
    code;
    statusCode;
...
exports.MessagesRepoError = MessagesRepoError;
class MessagesRepo {
    prisma;
    logger;
    constructor(cfg = {}) {
    ...
    async addMessage(input) {
        ...
        if (body === "" && attachmentArtifactIds.length < 1) {
            throw new MessagesRepoError("ML_MESSAGE_EMPTY", 400, "A message body or at least one attachment is required");
        }
        ...
        this.logger?.info("messages.added", {
    ...
    async getThread(input) {
        ...
        this.logger?.info("messages.thread.fetched", {
            prescription_id: prescriptionId,
    ...
    async markAsRead(input) {
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            const locked = await lockPrescription(tx, prescriptionId);
            assertActorCanAccessPrescription(locked, actor);
            const effectiveReadSeq = Math.max(actor.role === client_1.ActorRole.DOCTOR ? locked.doctorLastReadSeq : actor.role === client_1.ActorRole.PATIENT ? locked.patientLastReadSeq : 0, requestedReadSeq, locked.lastMessageSeq);
            await tx.prescription.update({
                where: { id: prescriptionId },
        ...
        this.logger?.info("messages.read", {
            prescription_id: prescriptionId,
            actor_role: actor.role,
            actor_wp_user_id: actor.wpUserId,
            unread_count_doctor: result.threadState.unreadCountDoctor,
...
async function lockPrescription(tx, prescriptionId) {
...
function resolveAuthorDoctorId(projection, actor) {
    if (actor.role !== client_1.ActorRole.DOCTOR) {
        return null;
    }
...
async function linkMessageAttachments(tx, input) {
    ...
    if (artifacts.length !== input.artifactIds.length) {
        throw new MessagesRepoError("ML_MESSAGE_ATTACHMENT_INVALID", 400, "One or more attachments were not found");
    }
    ...
    for (const artifactId of input.artifactIds) {
        ...
        if (artifact.deletedAt) {
            throw new MessagesRepoError("ML_MESSAGE_ATTACHMENT_INVALID", 400, "Attachment has been deleted");
        }
        if (artifact.kind !== client_1.ArtifactKind.MESSAGE_ATTACHMENT) {
            throw new MessagesRepoError("ML_MESSAGE_ATTACHMENT_INVALID", 400, "Only MESSAGE_ATTACHMENT artifacts can be attached to messages");
        }
        if (artifact.status !== client_1.ArtifactStatus.READY) {
            throw new MessagesRepoError("ML_MESSAGE_ATTACHMENT_INVALID", 400, "Attachment is not ready yet");
        }
        if (artifact.messageId) {
            throw new MessagesRepoError("ML_MESSAGE_ATTACHMENT_INVALID", 409, "Attachment is already linked to another message");
        }
        if (artifact.prescriptionId && artifact.prescriptionId !== input.prescriptionId) {
            throw new MessagesRepoError("ML_MESSAGE_ATTACHMENT_INVALID", 400, "Attachment belongs to another prescription");
        }
        if (input.actor.role !== client_1.ActorRole.SYSTEM) {
            if (artifact.ownerRole !== input.actor.role || artifact.ownerWpUserId !== input.actor.wpUserId) {
                throw new MessagesRepoError("ML_MESSAGE_ATTACHMENT_INVALID", 403, "Attachment owner does not match the message author");
            }
        }
        ...
        if (linked.count !== 1) {
            throw new MessagesRepoError("ML_MESSAGE_ATTACHMENT_INVALID", 409, "Attachment could not be linked safely");
        }
    }
}
function buildThreadUpdateDataAfterMessage(role, nextSeq, now) {
    const data = {
...
function buildThreadUpdateDataAfterRead(role, effectiveReadSeq) {
    switch (role) {
        case client_1.ActorRole.DOCTOR:
            return {
...
function normalizeActorInput(input) {
    ...
    if ((role === client_1.ActorRole.DOCTOR || role === client_1.ActorRole.PATIENT) && wpUserId == null) {
...
function normalizeMessageBody(value) {
    ...
    if (normalized.length > MAX_BODY_CHARS) {
        throw new MessagesRepoError("ML_MESSAGE_BAD_REQUEST", 400, `message.body exceeds ${MAX_BODY_CHARS} characters`);
    }
    return normalized;
}
