...
async function main() {
    ...
    const logger = new logger_1.NdjsonLogger("cron", process.env.ML_SITE_ID?.trim() || "bdpm-sync", process.env.SOSPRESCRIPTION_ENV?.trim() || "dev");
    const prisma = new client_1.PrismaClient();
    const downloadDir = opts.downloadDir ?? await (0, promises_1.mkdtemp)(node_path_1.default.join(node_os_1.default.tmpdir(), "bdpm-sync-"));
    const ctx = { logger, validCisSet: new Set() };
    logger.info("bdpm.sync.started", {
    ...
    try {
        ...
        if (!opts.keepDownloads) {
            await (0, promises_1.rm)(downloadDir, { recursive: true, force: true }).catch(() => undefined);
        }
    }
}
async function downloadAllDatasets(downloadDir, opts, logger) {
    const out = [];
    for (const definition of DATASETS) {
        const url = definition.buildUrl(opts);
        const filePath = node_path_1.default.join(downloadDir, definition.filename);
        await downloadFileWithRetry(url, filePath, opts.timeoutMs, logger);
        const fileStat = await (0, promises_1.stat)(filePath);
        logger.info("bdpm.download.completed", {
...
async function downloadFileWithRetry(url, filePath, timeoutMs, logger) {
    let lastError;
    for (let attempt = 1; attempt <= DOWNLOAD_RETRIES; attempt += 1) {
        try {
            ...
            lastError = err;
            logger.warning("bdpm.download.retry", {
                url,
                file_path: filePath,
                attempt,
            ...
            if (attempt < DOWNLOAD_RETRIES) {
                await (0, sleep_1.sleep)(attempt * 1_000);
            }
        }
    }
...
async function downloadFile(url, filePath, timeoutMs) {
    ...
    await (0, promises_2.pipeline)(node_stream_1.Readable.fromWeb(response.body), (0, node_fs_1.createWriteStream)(filePath));
}
async function truncateBdpmTables(prisma) {
...
async function importDatasetFile(definition, dataset, prisma, chunkSize, ctx) {
    const encoding = await detectFileEncoding(dataset.filePath);
    ctx.logger.info("bdpm.import.started", {
        dataset: dataset.key,
    ...
    for await (const line of iterateFileLines(dataset.filePath, encoding)) {
        ...
        if (chunk.length >= chunkSize) {
            ...
            if (lineNumber % 10_000 === 0) {
                ctx.logger.info("bdpm.import.progress", {
                    dataset: dataset.key,
    ...
    ctx.logger.info("bdpm.import.completed", {
        dataset: dataset.key,
...
async function* iterateFileLines(filePath, encoding) {
...
function splitTabColumns(line, expectedColumns) {
    const fields = [];
    let cursor = 0;
...
function parseSpecialty(fields, lineNumber) {
    const cis = requireString(fields[0], "cis", lineNumber);
    const denomination = requireString(fields[1], "denomination", lineNumber);
    return {
        cis,
        denomination,
        normalizedDenomination: (0, medicationSearch_1.normalizeSearchText)(denomination),
...
function parsePresentation(fields, lineNumber) {
    const cis = requireString(fields[0], "cis", lineNumber);
    ...
    return {
        ...
        cip13,
        label,
        normalizedLabel: (0, medicationSearch_1.normalizeSearchText)(label),
...
function parseComposition(fields, lineNumber) {
    const cis = requireString(fields[0], "cis", lineNumber);
...
function parseGenericGroupMember(fields, lineNumber) {
...
function parsePrescriptionCondition(fields, lineNumber) {
    const cis = requireString(fields[0], "cis", lineNumber);
    ...
    return {
        ...
        cis,
        conditionText,
        normalizedCondition: (0, medicationSearch_1.normalizeSearchText)(conditionText),
    };
}
function parseAvailabilityStatus(fields, lineNumber) {
    const cis = requireString(fields[0], "cis", lineNumber);
...
function parseTherapeuticInterest(fields, lineNumber) {
    const cis = requireString(fields[0], "cis", lineNumber);
...
function parseImportantInfo(fields, lineNumber) {
    const cis = requireString(fields[0], "cis", lineNumber);
...
function requireString(value, field, lineNumber) {
...
function parseOptionalFrenchDate(value) {
    const normalized = optionalString(value);
    if (!normalized) {
        return null;
    }
...
function parseRequiredInteger(value, field, lineNumber) {
...
function parseYesNoBoolean(value) {
    const normalized = (0, medicationSearch_1.normalizeSearchText)(value);
    if (normalized === "oui") {
        return true;
    }
    if (normalized === "non") {
...
function mapGenericTypeLabel(genericType) {
    switch (genericType) {
        case 0:
            return "princeps";
        case 1:
...
function stableRowId(kind, parts) {
...
async function collectRowCounts(prisma) {
    const [medications, presentations, compositions, genericGroupMembers, prescriptionConditions, availabilityStatuses, therapeuticInterests, importantInfos,] = await Promise.all([
...
function normalizeOptionalPath(value) {
    const normalized = String(value ?? "").trim();
    return normalized === "" ? undefined : node_path_1.default.resolve(normalized);
}
...
function findDatasetDefinition(key) {
