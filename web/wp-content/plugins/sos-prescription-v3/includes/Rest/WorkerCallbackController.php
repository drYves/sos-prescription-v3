...
final class WorkerCallbackController
{
    ...
    public function handle(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        if ($status === 'DONE') {
            $s3KeyRef = $job['s3_key_ref'] ?? null;
            if (!is_string($s3KeyRef) || $s3KeyRef === '') {
                return new WP_Error('ml_s3_key', 'Invalid s3_key_ref', ['status' => 400]);
            }

            $this->db->query($this->db->prepare(
                "UPDATE `{$this->jobsTable}` SET status='DONE',s3_key_ref=%s,completed_at=NOW(3),locked_at=NULL,lock_expires_at=NULL,locked_by=NULL WHERE job_id=%s AND site_id=%s",
                $s3KeyRef,
                $jobId,
                $this->siteId
            ));
            $this->logger->info('worker.callback.done', ['job_id' => $jobId], $reqId);
        } else {
