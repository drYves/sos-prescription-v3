<?php
// includes/Repositories/MedicationRepository.php
...
final class MedicationRepository
{
    ...
    public function search(string $q, int $limit = 20): array
    {
        ...
        if ($hasCip) {
            $selectDenomination = $hasCis ? 'c.denomination' : 'NULL AS denomination';
            ...
            $queryParams = array_merge($params, [$fetchLimit]);
            $prepared = $wpdb->prepare($sql, $queryParams);
        ...
        foreach ($rows as $row) {
            ...
            $presentation = isset($row['libelle_presentation']) ? (string) $row['libelle_presentation'] : '';
            $taux = isset($row['taux_remboursement']) ? (string) $row['taux_remboursement'] : '';
            $prix = isset($row['prix_ttc']) && $row['prix_ttc'] !== null && $row['prix_ttc'] !== ''
                ? (float) $row['prix_ttc']
                : null;

            $label = $denomination !== '' ? $denomination : ($presentation !== '' ? $presentation : 'Medicament');
        ...
        foreach ($out as $item) {
            $key = $item['cip13'] !== ''
                ? 'cip13:' . $item['cip13']
                : 'label:' . self::lower((string) $item['label']);
    ...
    public function table(string $q, int $page, int $perPage): array
    {
        ...
        if (!isset($wpdb) || !($wpdb instanceof \wpdb)) {
            return ['items' => [], 'total' => 0, 'page' => 1, 'perPage' => max(10, min(50, $perPage))];
        }
        ...
        foreach ($rows as $row) {
            $items[] = [
                ...
                'prix_ttc' => isset($row['prix_ttc']) && $row['prix_ttc'] !== null && $row['prix_ttc'] !== ''
                    ? (float) $row['prix_ttc']
                    : null,
            ];
        }
